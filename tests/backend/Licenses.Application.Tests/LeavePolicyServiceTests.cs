...
public sealed class LeavePolicyServiceTests
{
    ...
    public async Task UpdateVersionRejectsInvalidBalanceConfigurationWithClearMessages()
    {
        ...
        var missingBucket = await Assert.ThrowsAsync<InvalidOperationException>(() => _service.UpdateVersionAsync(version.Id, new(new DateOnly(2026, 1, 1), null, "BUSINESS_DAYS", true, 7, "CALENDAR_DAYS", 15m, "BLOCK", true, null), CancellationToken.None));
        var unexpectedBucket = await Assert.ThrowsAsync<InvalidOperationException>(() => _service.UpdateVersionAsync(version.Id, new(new DateOnly(2026, 1, 1), null, "BUSINESS_DAYS", true, 7, "CALENDAR_DAYS", 15m, "BLOCK", false, bucket.Id), CancellationToken.None));
    ...
    private static CreateLeavePolicyVersionCommand Command(DateOnly? effectiveFrom = null, DateOnly? effectiveTo = null, string dayCountMode = "BUSINESS_DAYS", string noticeMode = "CALENDAR_DAYS", string overlap = "BLOCK", decimal? maximum = 15m, bool consumesBalance = false, Guid? balanceBucketId = null) =>
        new(effectiveFrom ?? new DateOnly(2026, 1, 1), effectiveTo, dayCountMode, true, 7, noticeMode, maximum, overlap, consumesBalance, balanceBucketId);

    private static UpdateLeavePolicyVersionCommand UpdateCommand() =>
        new(new DateOnly(2026, 1, 1), null, "CALENDAR_DAYS", false, null, "CALENDAR_DAYS", null, "ALLOW", false, null);
